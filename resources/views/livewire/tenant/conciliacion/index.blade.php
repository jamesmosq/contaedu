    ...
    <div class="bg-gradient-to-br from-forest-900 via-forest-800 to-forest-950 px-6 py-8 lg:px-10">
        <div class="max-w-7xl mx-auto flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <div>
                ...
                <h1 class="font-display text-2xl font-bold text-white">Conciliación bancaria</h1>
                <p class="text-forest-300 text-sm mt-1">Verificación de movimientos bancarios</p>
            </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-2xl border border-cream-200 shadow-card overflow-hidden">
                        <div class="px-4 py-3 border-b border-cream-100 bg-slate-50">
                                ...
                                @foreach($reconciliations as $rec)
                                    <li>
                                        <button wire:click="selectReconciliation({{ $rec->id }})"
                                            class="w-full text-left px-4 py-3 hover:bg-slate-50 transition {{ $activeId === $rec->id ? 'bg-forest-50 border-l-4 border-forest-600' : '' }}">
                ...
                <div class="lg:col-span-2">
                    ...
                    @else
                        ...
                        <div class="space-y-4">
                            ...
                            {{-- Encabezado --}}
                            <div class="bg-white rounded-2xl border border-cream-200 shadow-card p-4">
                                <div class="flex items-start justify-between gap-4">
                                    ...
                                    <div class="flex items-center gap-2 shrink-0">
                                        <a href="{{ session('audit_mode')
                                            ? route('teacher.auditoria.conciliacion.pdf', ['tenantId' => session('audit_tenant_id'), 'id' => $rec->id])
                                            : (session('demo_mode')
                                                ? route('teacher.demo.conciliacion.pdf', ['demoId' => session('demo_tenant_id'), 'id' => $rec->id])
                                                : route('student.conciliacion.pdf', ['id' => $rec->id])) }}"
                                            target="_blank"
                                        ...
                                        @if(!$isLocked && !session('audit_mode') && !session('reference_mode'))
                                            <button wire:click="finalize"
                                                ...
                                                class="px-3 py-1.5 text-sm font-medium rounded-xl border transition
                                                    {{ $rec->isBalanced() ? 'bg-green-600 text-white hover:bg-green-700 border-green-600' : 'bg-slate-100 text-slate-400 border-cream-200 cursor-not-allowed' }}">
                                                {{ $rec->isBalanced() ? 'Finalizar' : 'Sin cuadrar' }}
                                            </button>
                                        @elseif($isLocked)
                                            <span class="px-3 py-1.5 text-sm font-semibold rounded-xl bg-green-100 text-green-700">
                                                ✓ Finalizada
                                            </span>
                                        @endif
                                    </div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-4 gap-3">
                                <div class="bg-white rounded-2xl border border-cream-200 shadow-card p-3 text-center">
                            ...
                            {{-- Fórmula educativa --}}
                            <div class="bg-amber-50 border border-amber-200 rounded-2xl p-3 text-xs text-amber-800 font-mono">
                            ...
                            <div class="bg-white rounded-2xl border border-cream-200 shadow-card overflow-hidden">
                                <div class="px-4 py-3 border-b border-cream-100 bg-slate-50 flex items-center justify-between">
                                    <div>
                                                        ...
                                                        <td class="px-4 py-2">
                                                            @if(!$isLocked && !session('audit_mode') && !session('reference_mode'))
                                                                <input type="checkbox"
                                                                    ...
                                                                    class="rounded border-slate-300 text-forest-600 cursor-pointer">
                                                            @else
                                ...
                                <div class="px-4 py-3 border-b border-cream-100 bg-slate-50 flex items-center justify-between">
                                    ...
                                    @if(!$isLocked && !session('audit_mode') && !session('reference_mode'))
                                        <button wire:click="openBankItemForm"
                                            class="text-xs px-3 py-1.5 bg-slate-800 text-white rounded-xl hover:bg-slate-700 transition">
                                            + Agregar
                                        </button>
                                    @endif
                                </div>
                            ...
                            @if($showBankItemForm && !session('audit_mode') && !session('reference_mode'))
                                <div class="bg-slate-50 border border-cream-200 rounded-2xl p-4 space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg">
                ...
                <div class="px-6 py-4 border-t border-cream-100 flex justify-end gap-3">
                    ...
                    <button wire:click="createReconciliation" wire:loading.attr="disabled"
                        class="px-4 py-2 bg-forest-800 text-white text-sm font-semibold rounded-xl hover:bg-forest-700 transition disabled:opacity-60">
