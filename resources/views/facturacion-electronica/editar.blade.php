<x-tenant-layout title="Editar Borrador">
    ...
    <div class="py-8">
        <div class="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <form method="POST" action="{{ fe_route('update', $factura) }}" id="form-fe"
                  x-data="facturaForm({{ $factura->detalles->map(fn($d) => [
                      ...
                      'iva'         => (int) $d->porcentaje_iva,
                      'totalLinea'  => (float) $d->total_linea,
                  ])->toJson() }})">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="resolucion_id" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                                @foreach($resoluciones as $res)
                                    <option value="{{ $res->id }}" @selected($factura->resolucion_id == $res->id)>
                                        {{ $res->prefijo ? $res->prefijo . ' — ' : '' }}{{ $res->numero_resolucion }} ({{ $res->rangoDisponible() }} disponibles)
                                    </option>
                                @endforeach
                            </select>
                        </div>
                        <div>
                            ...
                            <input type="date" name="fecha_emision"
                                   value="{{ old('fecha_emision', $factura->fecha_emision->toDateString()) }}" required
                        ...
                        <div>
                            ...
                            <input type="time" name="hora_emision"
                                   value="{{ old('hora_emision', \Carbon\Carbon::parse($factura->hora_emision)->format('H:i')) }}" required
                                   class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                </div>
                        ...
                        <select @change="cargarCliente($event)" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                            ...
                            @foreach($clientes as $cliente)
                                <option value="{{ $cliente->id }}"
                                    ...
                                    @selected($factura->cliente_id == $cliente->id)>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <select name="tipo_doc_adquirente" x-model="tipoDoc" required class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                                @foreach($tiposDocumento as $tipo)
                                    <option value="{{ $tipo->value }}" @selected($factura->tipo_doc_adquirente === $tipo->value)>
                        ...
                        <div>
                            ...
                            <p x-show="tipoDoc === '31'" class="mt-1 text-xs text-amber-600">
                                ⚠️ Para NIT debes incluir el dígito de verificación al final.
                            </p>
                        </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                            ...
                            <select name="medio_pago" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                                <option value="10" @selected($factura->medio_pago === '10')>10 — Efectivo</option>
                                <option value="42" @selected($factura->medio_pago === '42')>42 — Consignación / Débito</option>
                                <option value="48" @selected($factura->medio_pago === '48')>48 — Tarjeta de crédito/débito</option>
                                <option value="20" @selected($factura->medio_pago === '20')>20 — Cheque</option>
                            </select>
                        </div>
                        <div>
                            ...
                            <select name="forma_pago" x-model="formaPago" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                                <option value="1" @selected($factura->forma_pago === '1')>1 — Contado</option>
                                <option value="2" @selected($factura->forma_pago === '2')>2 — Crédito</option>
                            </select>
                        </div>
                        <div x-show="formaPago === '2'">
                            ...
                            <input type="date" name="fecha_vencimiento_pago"
                                   value="{{ old('fecha_vencimiento_pago', $factura->fecha_vencimiento_pago?->toDateString()) }}"
                                   class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm">
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white rounded-xl border border-slate-200 p-6 mb-6">
                    ...
                    <textarea name="notas" rows="2" class="w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
                              placeholder="Opcional">{{ old('notas', $factura->notas) }}</textarea>
                </div>
    ...
    @push('scripts')
    <script>
    function facturaForm(lineasIniciales) {
        return {
            clienteId: '{{ $factura->cliente_id ?? '' }}',
            tipoDoc: '{{ $factura->tipo_doc_adquirente }}',
            numDoc: '{{ $factura->num_doc_adquirente }}',
            nombre: '{{ addslashes($factura->nombre_adquirente) }}',
            email: '{{ $factura->email_adquirente }}',
            telefono: '{{ $factura->telefono_adquirente ?? '' }}',
            direccion: '{{ addslashes($factura->direccion_adquirente ?? '') }}',
            formaPago: '{{ $factura->forma_pago }}',
            lineas: lineasIniciales && lineasIniciales.length ? lineasIniciales : [{ producto_id: '', codigo: '', descripcion: '', cantidad: 1, precio: 0, descuento: 0, iva: 19, totalLinea: 0 }],
            subtotal: 0,
            totalIva: 0,
            total: 0,
